    ...
    run: async (bot, message, args) => {
    	...
    	if (giver !== null && cooldown - (Date.now() - giver) > 0 ) {
            ...
            .setAuthor({ name: `◠ Mechanic Centre [${bot.commands.size}] ◡`, iconURL: bot.user.displayAvatarURL()})
                ...
                .addFields(
                    { name: `➳ Bot Parts [${botCommandsList.length}]` , value: botCommandsList.map((data) => `${data}`).join(", "), inline: true },
                    { name: `➳ Fun Parts [${funCommandsList.length}]`, value: funCommandsList.map((data) => `${data}`).join(", "), inline: true }
                )
